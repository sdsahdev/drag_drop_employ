    ...
    return (
      <ScaleDecorator>
        <TouchableOpacity
          onLongPress={drag}
          disabled={isActive}
          style={{ backgroundColor: isActive ? 'gray' : 'rgb(0, 152, 247)', borderRadius: 10, margin: 10, alignItems: 'center', padding: 4, }}>
...
useEffect(()=> {
  ...
  return (
    <View>
      <TouchableOpacity onPress={() => navigation.navigate("AddEmploy")} style={{ backgroundColor: 'gray', padding: 10, alignItems: 'center', margin: 20, borderRadius: 12, }}>
        <Text style={{ color: '#fff' }}>Add Employ</Text>
      </TouchableOpacity>
      {datass.length === 0 || !datass.every(item => item.id) ? (
        <Text style={{ color: '#000', textAlign: 'center', textAlignVertical: 'center', justifyContent: 'center', fontSize: 20, }}>No data available</Text>
      ) : (
        <DraggableFlatList
          scrollEnabled
